...
export const filteringPattern = () => {
  ...
  const TableContent: React.FC = memo(() => {
    ...
    // Track if there are active filters or search term
    useEffect(() => {
     ...
     * - Only set error=true for actual fetch failures (network/server errors)
     * - Empty results (successful query with no matches) are NOT errors
     * - Empty results should show empty state UI, not error UI
     */
    useEffect(() => {
      let cancelled = false;
      ...
      const loadData = async () => {
        ...
        try {
          ...
          if (!cancelled) {
            ...
            // Don't set error for empty results - empty results are not errors
            // Empty results indicate successful query with no matches
            setError(false);
          }
        } catch (err) {
          if (!cancelled) {
            // Only set error=true for actual fetch failures
            setError(true);
    ...
    /**
     * Error template for Grid component
     *
     * Per UI guidelines:
     * - Error states: Show error image, retry action, and error messaging
     * - Only shown for actual fetch failures (network/server errors)
     * - Empty results (successful query with no matches) are handled separately below
     */
    ...
    /**
     * Empty state template for when query succeeds but returns no results
     *
     * Per UI guidelines:
     * - Empty states: Show guidance on adjusting filters/search, no error imagery
     * - Different messaging based on whether filters/search are active
     */
    const emptyStateTemplate = useMemo(() => {
      if (hasActiveFilters) {
        // User has applied filters/search - guide them to adjust
        return (
          <EmptyState>
      ...
      // No filters/search - initial empty state
      return (
        <EmptyState>
    ...
    return (
      <div className="overflow-auto flex-grow-1">
        {/* Show error template only for actual errors */}
        {error && !loading ? (
          <div className="d-flex align-items-center justify-content-center h-100">{errorTemplate}</div>
        ) : (
          <>
            {/* Show empty state when no data and not loading (successful query with no results) */}
            {!loading && !error && data.length === 0 ? (
              ...
              <Grid
                ...
                loading={loading}
                error={false}
                // Don't pass errorTemplate to Grid when showing empty state
...
export default {
  title: 'Components/Table/Filtering Pattern',
  parameters: {
    docs: {
      docPage: {
        tsxStory: true,

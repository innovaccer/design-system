...
() => {
  ...
  const fetchErrorRenderer = (fetchNextRowsFn) => {
    return (
      <div className="d-flex justify-content-center align-items-center my-5">
        <Text appearance="subtle">Failed to load data. Please try again.</Text>
        <LinkButton size="regular" icon="refresh" onClick={fetchNextRowsFn} className="ml-6">
          Try again
        </LinkButton>
      </div>
    );
  ...
  const retryFetchRenderer = () => {
    return (
      <div className="d-flex justify-content-center align-items-center my-5">
        <Text appearance="subtle">Loading in progress, please wait...</Text>
        <LinkButton size="tiny" icon="refresh" disabled={true} className="ml-6">
          Try again
        </LinkButton>
      </div>
    );
  ...
  const fetchData = (options) => {
    ...
    if (page === 3 && errorCount.current < 1) {
      ...
      <Card className="h-100 overflow-hidden">
        <Table
          ...
          infiniteScrollOptions={{
            fetchRowsCount: 50,
            fetchThreshold: 'balanced',
            fetchErrorRenderer: fetchErrorRenderer,
            retryFetchRenderer: retryFetchRenderer
          }}
        />
      </Card>
    </div>
...
export default {
  title: 'Components/Table/Infinite Scroll/Custom Error State',
  component: Table,
  parameters: {
    docs: {
      docPage: {

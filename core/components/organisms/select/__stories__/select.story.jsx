...
import { Select, Label, Button, Text } from '@/index';
...
const isDisabledOption = (label) => label === 'Ciprofloxacin' || label === 'Metformin';
...
const initialSingleValue = { label: 'Simvastatin', value: 'Simvastatin' };
...
export const all = () => {
  ...
  return (
    <div className="d-flex flex-wrap" style={{ gap: '2rem', maxWidth: 900 }}>
      <div style={{ minWidth: 260 }}>
        ...
        <Select
          onSelect={(option) => {
            ...
            setSingleValue(option || undefined);
          }}
          value={singleValue}
          triggerOptions={{
            onClear: () => setSingleValue(undefined),
          }}
        >
          <Select.List>
            {medicineList.map((item, key) => (
              <Select.Option
                ...
                disabled={isDisabledOption(item.label)}
              >
                {item.label}
              </Select.Option>
            ))}
      ...
      <div style={{ minWidth: 260 }}>
        <Label className="mb-2">Custom trigger with footer</Label>
        <Select
          onSelect={onSelectHandler}
        ...
        >
          <div className="px-4 pt-3">
            <Text weight="strong" size="small" className="mb-2">
              Medicines
            </Text>
          </div>
          <Select.List>
            {medicineList.slice(0, 7).map((item, key) => (
              <Select.Option
                key={key}
                option={{ label: item.label, value: item.value }}
                disabled={isDisabledOption(item.label)}
          ...
          <Select.Footer>
            <Button size="tiny" appearance="basic" type="button" onClick={() => action('viewAllMedicines')()}>
              View all medicines
            </Button>
          </Select.Footer>
        </Select>
...
const customCode = `() => {
  ...
  const isDisabled = (label) => label === 'Ciprofloxacin' || label === 'Metformin';
  ...
  const multiSelectRef = React.useRef(null);
  const [singleValue, setSingleValue] = React.useState({ label: 'Simvastatin', value: 'Simvastatin' });
  const [searchTerm, setSearchTerm] = React.useState('');
  ...
  return (
    <div className="d-flex flex-wrap" style={{ gap: '2rem', maxWidth: 900 }}>
      <div style={{ minWidth: 260 }}><Label className="mb-2">Single select</Label><Select onSelect={(opt) => setSingleValue(opt || undefined)} value={singleValue} triggerOptions={{ onClear: () => setSingleValue(undefined) }}><Select.List>{medicineList.map((item, key) => <Select.Option key={key} option={{ label: item.label, value: item.value }} disabled={isDisabled(item.label)}>{item.label}</Select.Option>)}</Select.List></Select></div>
      <div style={{ minWidth: 260 }}><Label className="mb-2">Multi select with search and footer</Label><Select ref={multiSelectRef} multiSelect={true} triggerOptions={{ setLabel }} onSelect={() => {}}><Select.SearchInput value={searchTerm} placeholder="Search" onChange={(v) => setSearchTerm(v || '')} onClear={() => setSearchTerm('')} />{filtered.length ? <Select.List>{filtered.map((item) => <Select.Option key={item.id || item.value} option={{ label: item.label, value: item.value, id: item.id }} disabled={isDisabled(item.label)}>{item.label}</Select.Option>)}</Select.List> : <Select.EmptyTemplate title="No results" description="Try a different search." />}<Select.Footer><Button size="tiny" appearance="basic" className="mr-2" type="button" onClick={onCancelMulti}>Cancel</Button><Button size="tiny" appearance="primary" type="button" onClick={onApplyMulti}>Apply</Button></Select.Footer></Select></div>
      <div style={{ minWidth: 260 }}><Label className="mb-2">Custom trigger with footer</Label><Select onSelect={() => {}} onToggle={(open) => setCustomOpen(open)} trigger={<Button appearance="basic" icon={isCustomOpen ? 'expand_less' : 'expand_more'} iconAlign="right">Choose option</Button>}><div className="px-4 pt-3"><Text weight="strong" size="small" className="mb-2">Medicines</Text></div><Select.List>{medicineList.map((item, key) => <Select.Option key={key} option={{ label: item.label, value: item.value }} disabled={isDisabled(item.label)}>{item.label}</Select.Option>)}</Select.List><Select.Footer><Button size="tiny" appearance="basic" type="button">View all medicines</Button></Select.Footer></Select></div>
    </div>
  );
};`;

...
export const all = () => {
  ...
  return (
    <div className="d-flex flex-wrap" style={{ gap: '2rem', maxWidth: 900 }}>
      ...
      <div style={{ minWidth: 260 }}>
        <Label className="mb-2">Multi select without footer</Label>
        <Select
          multiSelect={true}
          value={[{ label: 'Ciprofloxacin', value: 'Ciprofloxacin' }]}
          onSelect={onSelectHandler}
          triggerOptions={{ setLabel: setLabelHandler }}
        >
          <Select.List>
...
const customCode = `() => {
  ...
  React.useEffect(() => {
    if (searchTerm.trim() === '') {
      ...
      setFiltered(searchList.filter((m) => m.label.toLowerCase().includes(searchTerm.toLowerCase())));
    }
  }, [searchTerm]);
      ...
      <div style={{ minWidth: 260 }}>
        ...
        >
          ...
          {filtered.length ? (
            <Select.List>
              {filtered.map((item) => (
                <Select.Option key={item.id || item.value} option={{ label: item.label, value: item.value, id: item.id }}>
                  {item.label}
                </Select.Option>
              ))}
            </Select.List>

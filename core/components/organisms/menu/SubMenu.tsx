...
export const SubMenu = (props: SubMenuProps) => {
  ...
  const onTriggerBlur = (e: React.FocusEvent) => {
    ...
    (submenuTrigger as React.ReactElement<any>)?.props?.onBlur?.(e);
  };

  const isTriggerDisabled = !!(submenuTrigger as React.ReactElement<any>)?.props?.disabled;
  ...
  if (React.isValidElement(submenuContent)) {
    const { on, children, onToggle, open: consumerOpen } = submenuContent?.props as any;
    subMenuElement = React.cloneElement(submenuContent as React.ReactElement, {
      ...(submenuContent?.props as any),
      on: on || 'hover',

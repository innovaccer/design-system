...
export const TimePickerWithSearch = (props: TimePickerDropdownProps) => {
  ...
  const [selectedOptionValue, setSelectedOptionValue] = React.useState<string | undefined>(undefined);
  ...
  const [fetchScrollIndex, setFetchScrollIndex] = React.useState<number | undefined>(undefined);
  ...
  const selectedIndex = React.useMemo(() => {
    if (!selectedOptionValue) return -1;
    return dropdownOptionList.findIndex((opt) => opt.value === selectedOptionValue);
  }, [selectedOptionValue, dropdownOptionList]);
  ...
  const effectiveScrollIndex = isFetchMode && fetchScrollIndex !== undefined ? fetchScrollIndex : scrollIndex;
  ...
  React.useEffect(() => {
    if (!openPopover) {
      ...
      setFetchScrollIndex(undefined);
      return;
    }

    const timer = setTimeout(() => {
      scrollToIndex(selectedIndex !== -1 ? selectedIndex : effectiveScrollIndex);
    }, 100);
  ...
  }, [openPopover, searchTerm, fetchTimeOptions]);

  React.useEffect(() => {
    if (openPopover && effectiveScrollIndex >= 0) {
      const timer = setTimeout(() => scrollToIndex(effectiveScrollIndex), 50);
      return () => clearTimeout(timer);
    }
  }, [effectiveScrollIndex]);
  ...
  const onSelectHandler = (option?: OptionType | OptionType[]) => {
    ...
    setSelectedOptionValue(option.value as string);
  ...
  const showEmptyState = !fetchTimeOptions
    ...
    : isFetchMode && !fetchLoading && fetchedOptions.length === 0;

  const selectedValue = React.useMemo(() => {
    if (selectedIndex < 0 || !dropdownOptionList[selectedIndex]) return undefined;
    return { label: dropdownOptionList[selectedIndex].label, value: dropdownOptionList[selectedIndex].value };
  }, [selectedIndex, dropdownOptionList]);
  ...
  return (
    ...
    >
      ...
      {showEmptyState ? (
        ...
        <div ref={listRef}>
          <Select.List aria-label={optionsAriaLabel}>
            {activeOptions.map((option, index) => {
              const isActive = index === effectiveScrollIndex && searchTerm !== '';

...
export const TimePickerAsDropdown = (props: TimePickerDropdownProps) => {
  ...
  React.useEffect(() => {
    console.log('open', open, '///', scrollFlag);
    if (open && scrollFlag) {
      const selectedIndex = dropdownOptionList.findIndex((option) => option.selected === true);
      console.log('re-renderr', dropdownOptionList[tabIndex], 'selectedIndex', selectedIndex);

      setTimeout(() => {
        scrollToOptionIndex(tabIndex);
      }, 500);
    }
  }, [open, scrollFlag]);
  ...
  const getOptionList = (searchTerm: string) => {
    ...
    console.log('dropdown222', dropdownOptionList);
    // const selectedIndex = dropdownOptionList.findIndex((option) => option.selected === true);
    // if (selectedIndex !== -1) {
    //   indexValue = selectedIndex;
    // }
    // console.log('selectedIndex', selectedIndex, indexValue);
    scrollFlag = false;

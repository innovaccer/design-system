      ...
      setTimeout(() => {
        if (textareaRef.current) {
          ...
          if (selection) {
            ...
            // Get all child nodes after the DOM has been updated
            const childNodes = Array.from(textareaRef.current.childNodes);

            // If there are nodes after the removed mention in the new content
            if (indexToRemove < newContent.length && childNodes.length > indexToRemove) {
              // Place cursor after the node at the same index in the DOM
              const targetNode = childNodes[indexToRemove];

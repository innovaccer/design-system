...
export const ChatInput: React.FC<ChatInputProps> = (props: ChatInputProps) => {
  ...
  const onCloseHandler = (mentionValue: string) => {
    ...
    setContent((prevContent) => {
      ...
      const indexToRemove = prevContent.findIndex(
        (item) => item.type === 'mention' && item.data.value === mentionValue
      );

      if (indexToRemove === -1) return prevContent;

      // Create a new array without the mention at the found index
      const newContent = [...prevContent];
      newContent.splice(indexToRemove, 1);

      // Update the text content to reflect the removal
      setTimeout(() => {
        if (textareaRef.current) {

...
export const TabsWrapper = (props: TabsWrapperProps) => {
  ...
  const firstEnabledIndex = tabs.findIndex((tab) => !(tab.props as { disabled?: boolean }).disabled);
  const isFocusedIndexValid =
    focusedIndex >= 0 &&
    focusedIndex < totalTabs &&
    !(tabs[focusedIndex]?.props as { disabled?: boolean } | undefined)?.disabled;
  ...
  const focusAdjacentTab = (fromIndex: number, direction: 1 | -1) => {
    ...
    for (let steps = 0; steps < totalTabs; steps += 1) {
      if (!(tabs[nextIndex].props as { disabled?: boolean }).disabled) {

...
export class Handle extends React.Component<InternalHandleProps, HandleState> {
  ...
  changeValue = (newValue: number, callback = this.props.onChange) => {
    ...
    const isDecrementing = typeof currentValue === 'number' && newValue < currentValue;

    let snapped: number;

    if (isIncrementing || isDecrementing) {
      // For directional movement (keyboard), we need to find the next valid step
      // If current value is on-grid, use newValue brackets to find next step
      // If current value is off-grid, use currentValue brackets to snap to nearest
  ...
  handleKeyDown = (event: React.KeyboardEvent<HTMLSpanElement>) => {
    ...
    if (keyCode === Keys.ARROW_LEFT || keyCode === Keys.ARROW_DOWN) {
      ...
      this.changeValue(value - largeStep);
      event.preventDefault();

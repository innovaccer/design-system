...
describe('Checkbox component', () => {
  const mapper: Record<string, any> = {
    ...
    value: valueHelper(StringValue, { required: true }),
    onChange: valueHelper(FunctionValue, { required: true }),
    warpLabel: valueHelper(BooleanValue, { required: true, iterate: true }),
  };
  const testFunc = (props: Record<string, any>): void => {
    const attr = filterUndefined(props) as Props;
...
describe('Checkbox component', () => {
  ...
  it('renders label with warpLabel applied', () => {
    const { getByTestId } = render(<Checkbox label={StringValue} warpLabel={true} />);
    expect(getByTestId('DesignSystem-Checkbox-Label')).toHaveClass('ellipsis');
  });

  it('renders label without warpLabel applied', () => {
    const { getByTestId } = render(<Checkbox label={StringValue} warpLabel={false} />);
    expect(getByTestId('DesignSystem-Checkbox-Label')).toHaveClass('ellipsis--noWrap');
  });

---
title: Pill
description: Pill is used to highlight the count of an item.
tabs: ['Usage', 'Code']
showMobile: true
---

Pill is used to highlight the count of an item.

### Appearances

A pill comes in **9 appearances** and **2 styles**.

#### Solid style
Solid style is used to highlight the number of items in reference to a particular component so that the count gets highlighted easily e.g. count over the notification icon.
<Preview name='components-pills-variants-appearance--appearance'/>
<br/>

#### Subtle style
Subtle style is favorable when a lot of pills have to be used to show count against entities. In that case, solid style becomes a bit overwhelming e.g. count in vertical navigation items.
<Preview name='components-pills-subtle-alert--subtle-alert'/>
<br/>

### Properties
<br/>

<table style="width: 100%">
  <tbody>
    <tr>
      <th style="width:33%; text-align: left;">Property</th>
      <th style="width:33%; text-align: left;">Value(s)</th>
      <th style="width:33%; text-align: left;">Default value</th>
    </tr>
    <tr style="vertical-align: top">
      <td>Appearance</td>
      <td>
        <ul>
            <li>Jal</li>
            <li>Stone</li>
            <li>Neem</li>
            <li>Haldi</li>
            <li>Mirch</li>
            <li>Tawak</li>
            <li>Nimbu</li>
            <li>Neel</li>
            <li>Jamun</li>
        </ul>
      </td>
      <td>Stone</td>
    </tr>
    <tr style="vertical-align: top">
      <td>Subtle<br/><em>(optional)</em></td>
      <td>
          <ul>
              <li>True</li>
              <li>False</li>
          </ul>
      </td>
      <td>-</td>
    </tr>
    <tr style="vertical-align: top">
      <td>Max width</td>
      <td>160px</td>
      <td>-</td>
    </tr>
    <tr style="vertical-align: top">
      <td>Corner radius</td>
      <td>20px</td>
      <td>-</td>
    </tr>
    <tr style="vertical-align: top">
      <td>Padding<br/><em>(top, right, bottom, left)</em></td>
      <td>2px, 6px, 2px, 6px</td>
      <td>-</td>
    </tr>
  </tbody>
</table>
<br/>

### Usage

<br/>

#### Pills vs Badges

Pills are used to display count of entities while badges are used for tagging entities.

![Pills vs Badges](./images/pills-1.png)

<br/>
<br/>
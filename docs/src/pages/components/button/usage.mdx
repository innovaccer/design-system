...
Basic button is the default button style used for most actions on a page. Use the fill style when there are limited actions and the button needs to stand out on its own. Outline style should be used for lower-emphasis (secondary) actions, especially when accompanied by a primary or alert button with higher prominence in the group.
Outline style is used for lower emphasis (secondary) affirmitive actions. For example when there is already a primary button or alert button has higher prominence in the group.

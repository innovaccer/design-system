...
Cypress.Commands.add('livePreview', () => {
  cy.get('[data-test=Docs-Main--Row]').then((page) => {
    if (page.find('[data-test=Docs-live--provider]').length) {
      cy.wait(3000);
      cy.get('[data-test=Docs-live--provider]').find('[data-test=Docs-error-message]').should('not.exist');
    }
    cy.get('[data-test=Docs-inner--container]').find('[data-test=Docs-propTable-error]').should('not.exist');
  });
});
...
Cypress.Commands.add('tabsVisit', () => {
  cy.get('[data-test=Docs-Main--Row]').then(($body) => {
    if ($body.find('[data-test=DesignSystem-Tabs--Tab]').length > 1) {
